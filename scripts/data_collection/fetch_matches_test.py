#!/usr/bin/env python3
"""
Test semplificato per fetch_matches.py
"""
import os
import sys
...
def main():
    print("Test script per data collection")
    
    # Verifica variabili d'ambiente
    print("Verifica variabili d'ambiente:")
    print(f"FOOTBALL_API_KEY: {'✓ presente' if os.getenv('FOOTBALL_API_KEY') else '✗ mancante'}")
    ...
    try:
        if os.environ.get('FIREBASE_CREDENTIALS'):
            ...
            # Aggiorna lo stato
            ref = db.reference('health/data_collection')
            ref.update({
