logger.debug(f"Errore nell'ottenere dati xG da FBref: {e}")
    ...
    def _normalize_transfermarkt(self, match_data: Dict[str, Any], output: Dict[str, Any]) -> None:
        """
        Normalizza i dati da Transfermarkt.
        
        Args:
            match_data: Dati grezzi della partita da Transfermarkt.
            output: Dizionario di output da popolare.
        """
        try:
            # ID partita
            if 'id' in match_data:
                output['match_id'] = str(match_data['id'])
                output['source_ids']['transfermarkt'] = str(match_data['id'])
            ...
            # Stato partita
            if 'status' in match_data:
                output['status'] = self._map_transfermarkt_status(match_data['status'])
            
            # Punteggio
            if 'result' in match_data:
                try:
                    output['score']['home'] = int(match_data['result'].get('home', 0))
                    output['score']['away'] = int(match_data['result'].get('away', 0))
    ...
    def _map_transfermarkt_status(self, status: str) -> str:
        """
        Mappa lo stato della partita da Transfermarkt a formato standard.
        
        Args:
            status: Stato della partita nel formato Transfermarkt.
        ...
        if 'finished' in status or 'beendet' in status:
            return 'finished'
        elif 'cancel' in status or 'abgesagt' in status:
            return 'cancelled'
        elif 'postponed' in status or 'verschoben' in status:
            return 'postponed'
        elif 'live' in status or 'läuft' in status:
            return 'in_progress'
        elif 'scheduled' in status or 'angesetzt' in status:
            return 'scheduled'
        
        return 'unknown'
    
    def _map_sofascore_status(self, status: str) -> str:
        """
        Mappa lo stato della partita da SofaScore a formato standard.
        
        Args:
            status: Stato della partita nel formato SofaScore.
    ...
    def _map_footystats_status(self, status: str) -> str:
        """
        Mappa lo stato della partita da FootyStats a formato standard.
        
        Args:
            status: Stato della partita nel formato FootyStats.
        ...
        if 'finished' in status or 'fulltime' in status or 'complete' in status:
            return 'finished'
            ...
            return 'cancelled'
        elif 'scheduled' in status or 'upcoming' in status:
            return 'scheduled'
        
        return 'unknown'
    
    def _map_whoscored_status(self, status: str) -> str:
        """
        Mappa lo stato della partita da WhoScored a formato standard.
        
        Args:
            status: Stato della partita nel formato WhoScored.
    ...
    def _map_generic_status(self, status: str) -> str:
        """
        Mappa uno stato generico della partita a formato standard.
        
        Args:
            status: Stato della partita in qualsiasi formato.
        ...
        if any(x in status for x in ['finish', 'complete', 'ft', 'ended']):
            return 'finished'
        elif any(x in status for x in ['live', 'progress', 'ongoing', 'halftime', 'ht']):
            return 'in_progress'
        elif any(x in status for x in ['postpone', 'delay']):
            return 'postponed'
        elif any(x in status for x in ['cancel', 'off']):
            return 'cancelled'
        elif any(x in status for x in ['schedule', 'upcoming', 'fixture']):
            return 'scheduled'
        elif any(x in status for x in ['suspend', 'interrupt']):
            return 'suspended'
        elif any(x in status for x in ['abandon']):
            return 'abandoned'
        
        return 'unknown'    def enrich_match_data(self, match_data: Dict[str, Any]) -> Dict[str, Any]:
        """
    ...
    def _get_lineups_from_source(self, source_id: str, source: str) -> Dict[str, Any]:
        """
        Ottiene formazioni da una fonte specifica.
        ...
        Returns:
            Dizionario con le formazioni della partita.
        """
        if source == 'api_football':
            return self.api_football.get_fixture_lineups(int(source_id))
        
        elif source == 'flashscore':
            match_details = self.flashscore.get_match_details(source_id)
            return match_details.get('lineups', {}) if match_details else {}
        
        elif source == 'sofascore':
            return self.sofascore.get_match_lineups(source_id)
        
        elif source == 'fbref':
            return self.fbref.get_match_lineups(source_id)
        
        elif source == 'whoscored':
            return self.whoscored.get_match_lineups(source_id)
        
        return {}
    
    def _get_stats_from_source(self, source_id: str, source: str) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dizionario con le statistiche della partita.
        """
        if source == 'api_football':
            return self.api_football.get_fixture_statistics(int(source_id))
        
        elif source == 'flashscore':
            match_details = self.flashscore.get_match_details(source_id)
            return match_details.get('statistics', {}) if match_details else {}
        
        elif source == 'sofascore':
            return self.sofascore.get_match_statistics(source_id)
        
        elif source == 'fbref':
            return self.fbref.get_match_statistics(source_id)
        
        elif source == 'understat':
            return self.understat.get_match_stats(source_id)
        
        elif source == 'whoscored':
            return self.whoscored.get_match_stats(source_id)
        
        elif source == 'footystats':
            return self.footystats.get_match_stats(source_id)
...
class MatchProcessor:
    ...
    def enrich_match_data(self, match_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for team_key, team_data in [('home_team', enriched_data['home_team']), ('away_team', enriched_data['away_team'])]:
                if team_data.get('id') and not team_data.get('short_name'):
                    try:
                        ...
                        # Prova prima con Football-Data
                        if 'football_data' in enriched_data['source_ids']:
                            try:
                                team_details = self.football_data_api.get_team(int(team_id))
                                if team_details:
                                    team_data['name'] = team_details.get('name', team_data['name'])
                        ...
                        # Se non ha funzionato, prova con API-Football
                        if not team_data.get('short_name') and 'api_football' in enriched_data['source_ids']:
                            try:
                                team_details = self.api_football.get_team(int(team_id))
                                if team_details:
                                    team_data['name'] = team_details.get('name', team_data['name'])
            ...
            if not enriched_data.get('odds') and enriched_data['status'] in ['scheduled', 'unknown']:
                try:
                    # Prova a ottenere quote da API-Football
                    if 'api_football' in enriched_data['source_ids']:
                        fixture_id = enriched_data['source_ids']['api_football']
                        odds_data = self.api_football.get_odds(int(fixture_id))
                        if odds_data:
                            enriched_data['odds'] = odds_data
                except Exception as e:
                    logger.warning(f"Errore nell'ottenimento delle quote per la partita {enriched_data['match_id']}: {e}")
            ...
            if not enriched_data.get('lineups') and enriched_data['status'] in ['in_progress', 'finished']:
                try:
                    ...
                    if 'api_football' in enriched_data['source_ids']:
                        fixture_id = enriched_data['source_ids']['api_football']
                        lineups_data = self.api_football.get_fixture_lineups(int(fixture_id))
                        if lineups_data:
                            enriched_data['lineups'] = lineups_data
                    elif 'flashscore' in enriched_data['source_ids']:
                        match_id = enriched_data['source_ids']['flashscore']
                        match_details = self.flashscore.get_match_details(match_id)
                        if match_details and 'lineups' in match_details:
                            enriched_data['lineups'] = match_details['lineups']
                except Exception as e:
                    logger.warning(f"Errore nell'ottenimento delle formazioni per la partita {enriched_data['match_id']}: {e}")
            ...
            if not enriched_data.get('statistics') and enriched_data['status'] in ['in_progress', 'finished']:
                try:
                    ...
                    if 'api_football' in enriched_data['source_ids']:
                        fixture_id = enriched_data['source_ids']['api_football']
                        stats_data = self.api_football.get_fixture_statistics(int(fixture_id))
                        if stats_data:
                            enriched_data['statistics'] = stats_data
                    elif 'flashscore' in enriched_data['source_ids']:
                        match_id = enriched_data['source_ids']['flashscore']
                        ...
                        if match_details and 'statistics' in match_details:
                            enriched_data['statistics'] = match_details['statistics']
                except Exception as e:
                    logger.warning(f"Errore nell'ottenimento delle statistiche per la partita {enriched_data['match_id']}: {e}")
                except Exception as e:
                    logger.warning(f"Errore nell'ottenimento delle statistiche per la partita {enriched_data['match_id']}: {e}")
